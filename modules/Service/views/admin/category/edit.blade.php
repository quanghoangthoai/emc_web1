...
<div class="page-header-content header-elements-md-inline">
    <div class="page-title d-flex">
        <h4><i class="icon-tree7 mr-2"></i> <span class="font-weight-semibold">Dịch vụ</span> - Danh mục #{{ $category_edit['id'] }}</h4>
...
<div class="card-body">
    <div class="row">
        <div class="col-xl-4">
            @include('Service::admin.category.component_edit')
        </div>
        <div class="col-xl-8">
            @include('Service::admin.category.component_list')
        </div>
    </div>
</div>
@endsection
...
<script>
    function change_order(el)
    {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('mod_service.admin.ajaxChangeOrderCategory') }}",
            data: {
                _token: _token,
                id: id,
                order: order
            ...
            $(html).on('click', function(e){
                if (!inProcess) {
                    if (typeof $(this).attr('checked') !== typeof undefined) {
                        ...
                        $.ajax({
                            type: 'post',
                            url: "{{ route('mod_service.ajax.changeStatusCategory') }}",
                            data: {
                                _token: _token,
                                id: $(this).data('id'),
                                status: 0
