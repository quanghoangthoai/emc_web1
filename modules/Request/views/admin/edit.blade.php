...
<div class="page-header-content header-elements-md-inline">
    <div class="page-title d-flex">
        <h4><i class="fa fa-file-alt mr-2"></i> <span class="font-weight-semibold">Yêu cầu</span> - Sửa</h4>
...
<form action="{{ route('mod_request.admin.post_edit_request', $iRequest['id']) }}" enctype="multipart/form-data" method="post">
    {{ csrf_field() }}
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        ...
                        <option value="{{ $key }}" {{ old('status', $iRequest['status']) == $key ? 'selected' : '' }}>
                            {{ $value }}</option>
                        @endforeach
                        @endif
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div>
                        <textarea placeholder="Nhập nội dung ghi chú" style="height: 200px" name="note" type="text" class="form-control">{{ old('note', $iRequest['note']) }}</textarea>
                    </div>
                </div>
                <div class="text-center">
                    <button type="submit" class="btn btn-info btn-sm">Lưu lại</button>
                    @if($iRequest['status'] == 1 && $iRequest['isOrderCreated'] == 0)
                    <a href="{{ View::exists('Order::admin.order') ? route('mod_order.admin.order',$iRequest['id']) : '#' }}" class="btn btn-info btn-sm">Tạo đơn hàng</a>
                    @endif
                </div>
            </div>
        </div>
