...
<div class="page-header-content header-elements-md-inline">
    <div class="page-title d-flex">
        <h4><i class="fa fa-file-alt mr-2"></i> <span class="font-weight-semibold">Slide</span> - Danh sách</h4>
        <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
    </div>
    <div class="header-elements d-none">
        <div class="d-flex justify-content-center">
            <a href="{{route('mod_slide.admin.add_slide')}}" class="btn btn-primary btn-sm">Thêm slide</a>
        </div>
    </div>
</div>
@endsection
        ...
        <tbody>
            ...
            <tr>
                <td class="text-center">
                    <span style="display:none;">{{ $iSlide['order'] }}</span>
                    <select data-min="{{ $minOrder }}" data-max="{{ $maxOrder }}" data-order="{{ $iSlide['order'] }}" data-id="{{ $iSlide['id'] }}" class="form-control changOrderPage"></select>
                </td>
                <td class="text-center"><a href="#"><strong>{{ $iSlide['title'] }}</strong></a></td>
                <td class="text-center"><a><strong>{{ $iSlide['description'] }}</strong></a></td>
                <td class="text-center"><a href="{{ $iSlide['link']  }}"><strong>{{ $iSlide['link'] }}</strong></a></td>
                <td class="text-center">
                    <div class="form-check form-check-switchery form-check-switchery-sm">
                        <label class="form-check-label">
                            <input data-id="{{ $iSlide['id'] }}" type="checkbox" class="form-input-switchery" {{ $iSlide['status'] ? 'checked' : '' }}>
                        </label>
                    </div>
                </td>
                <td class="text-center">
                    <a href="{{ route('mod_slide.admin.edit_slide', $iSlide['id']) }}" class="text-warning mr-2" data-popup="tooltip" title="Sửa"><i class="fa fa-edit"></i></a>
                    <a href="javascript:;" onclick="askToDelete(this);return false;" data-href="{{ route('mod_slide.admin.delete_slide', $iSlide['id']) }}" class="text-danger" data-popup="tooltip" title="Xóa"><i class="fas fa-trash-alt"></i></a>
                </td>
            </tr>
            @endforeach
        </tbody>
...
<script>
            ...
            $(html).on('click', function(e){
                if (!inProcess) {
                    if (typeof $(this).attr('checked') !== typeof undefined) {
                        ...
                        $.ajax({
                            type: 'post',
                            url: "{{ route('mod_slide.ajax.changeStatus') }}",
                            data: {
                                _token: _token,
                                id: $(this).data('id'),
                                status: 0
    ...
    function change_order(el)
    {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('mod_slide.admin.ajaxChangeOrderSlide') }}",
            data: {
                _token: _token,
                id: id,
                order: order
